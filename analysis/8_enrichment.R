...
for (method in c("NegBinom", "tensorqtl")){
  ...
  for (idx in seq_along(tmp$annot)){
    ...
    if (nrow(meta_dat) > 0){
      ...
      tmp$meta_se[[idx]] <- metares$seTE.fixed
    }
  }
...
nb_or <- baseline_meta_celltype %>% filter(method == "NegBinom") %>% pull(meta_OR)
nb_or_se <- baseline_meta_celltype %>% filter(method == "NegBinom") %>% pull(meta_se)
lm_or <- baseline_meta_celltype %>% filter(method != "NegBinom") %>% pull(meta_OR)
lm_or_se <- baseline_meta_celltype %>% filter(method != "NegBinom") %>% pull(meta_se)

stats <- (log(nb_or) - log(lm_or))/sqrt((nb_or_se^2 + lm_or_se^2))
...
atac_meta <- data.frame(sc_bulk = c("sc-eQTL", "bulk-eQTL"),
                       meta_OR=NA, meta_lower=NA, meta_upper=NA, seTE=NA)

for (idx in 1:2){
  dat <- list(atac_meta_celltype, atac_meta_allcells)[[idx]]
  ...
  m.gen_bin <- metagen(TE = TE,
                       ...
                       random = FALSE,
                       title = "Enrichment (Pre-calculated)")
  atac_meta$meta_OR[idx] <- exp(m.gen_bin$TE.fixed)
  atac_meta$meta_lower[idx] <- exp(m.gen_bin$lower.fixed)
  atac_meta$meta_upper[idx] <- exp(m.gen_bin$upper.fixed)
  atac_meta$seTE[idx] <- m.gen_bin$seTE.fixed
}

atac_meta
# meta:
# 14 cell types: 1.988394 [1.902294; 2.078390]
# allcells: 1.723682 [1.690670; 1.757339]

stats <- (log(atac_meta$meta_OR[1]) - log(atac_meta$meta_OR[2]))/sqrt(sum(atac_meta$seTE^2))
pnorm(abs(stats), lower.tail = F) * 2

forest_plot_theme(atac_meta %>% 
                    mutate(sc_bulk = fct_reorder(sc_bulk, meta_OR)) %>% 
...
epimap_meta_celltype <- epimap_meta_celltype %>% 
  mutate(sc_bulk = as.factor(ifelse(celltype == "allcells", "bulk-eQTL", "sc-eQTL"))) %>% 
  filter(!grepl("^PBMC", annot))

View(epimap_meta_celltype)
# forest plot
forest_plot_theme(epimap_meta_celltype %>% 
                    filter(method == "NegBinom") %>% 
                    mutate(celltype=ifelse(celltype=="allcells", "Bulk-eQTL", celltype)) %>% 
                    group_by(celltype) %>% 
                    mutate(enhancer_or = meta_OR[category == "enhancer"]) %>% ungroup() %>% 
                    mutate(celltype = fct_reorder(celltype, enhancer_or)) %>% 
                    ggplot(aes(y = celltype, x = meta_OR, color = category)) +
                    geom_point(shape = 18, size = 2, position = position_dodge(width=0.5)) +  
                    geom_errorbarh(aes(xmin = meta_lower, xmax = meta_upper),
                                   height = 0.25, 
                                   position = position_dodge(width=0.5)) +
                    geom_vline(xintercept = 1, color = "red", 
                               linetype = "dashed", cex = 1, alpha = 0.5, linewidth=0.6) +
                    scale_color_manual(values = c("enhancer"="#009E73",
                                                  "promoter"="#E69F00")) +
                    xlab("Odds Ratio (95% CI)") + 
                    ylab(" ") + 
                    theme_bw() +
                    ggtitle("Cell type-specific enhancer or promoter (EpiMap)"))+
  theme(axis.text.x = element_text(angle=35, hjust = 1),
        legend.position = c(.95, .35),
        legend.justification = c("right", "top"),
        legend.box.just = "right",
...
ggsave2(paste0(plots_dir, "/enrich_sm/",
               "jaxqtl_allegenes_enrich_epimap.png"),
        width = onecol_width*1.2, height = onecol_height*1.8, units = 'cm', dpi = 300)

# # meta across celltypes: all cell type specific versus bulk

epimap_meta <- expand.grid(sc_bulk = c("sc-eQTL", "bulk-eQTL"),
            category = c("enhancer", "promoter"),
            method = c("NegBinom", "tensorqtl")) %>% 
  mutate(meta_OR=NA, meta_lower=NA, meta_upper=NA, seTE=NA)

for (idx in seq_along(epimap_meta$sc_bulk)){
  which_sc_bulk <- epimap_meta$sc_bulk[[idx]]; 
  which_annot <- epimap_meta$category[[idx]]; 
  which_method <- epimap_meta$method[[idx]]; 
  
  
  if (which_sc_bulk == "bulk-eQTL"){
...
forest_plot_theme(epimap_meta %>% 
                    filter(method == "NegBinom") %>% 
                    mutate(sc_bulk = fct_reorder(sc_bulk, meta_OR)) %>% 
                    ggplot(aes(y = sc_bulk, x = meta_OR, color = category)) +
                    geom_point(shape = 18, size = 1.5, position = position_dodge(width=0.5)) +  
                    geom_errorbarh(aes(xmin = meta_lower, xmax = meta_upper),
                                   height = 0.25, 
                                   position = position_dodge(width=0.5)) +
